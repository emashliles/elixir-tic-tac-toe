defmodule WinOrTieCheckerTests do
  ...
  test "can detect a win if a row is taken by a player" do
    assert WinOrTieChecker.check_if_win_or_tie([["O","O","3"],["X","X","X"],["7","8","9"]]) == :win
  end
  ...
  test "retruns no_win_detected if none is found" do
    assert WinOrTieChecker.check_if_win_or_tie([["O","O","3"],["X","5","X"],["7","8","9"]])  == :continue
  end
  ...
  test "can detect a win if player takes a column" do
    assert WinOrTieChecker.check_if_win_or_tie([["1","2","X"], ["4", "O", "X"], ["O", "O", "X"]]) == :win
  end

  test "can detect a tie" do
    assert WinOrTieChecker.check_if_win_or_tie([["O", "O", "X"], ["X", "X", "O",], ["O", "X", "O"]]) == :tie
  end
